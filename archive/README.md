# Irish Trad Session Archive

This folder is where the archived session PDFs live. The list below is generated automatically by the archive workflow.

Each filename is irish-trad-session_yyyy-mm-dd_hh-mm-ss.pdf.

<!-- ARCHIVE_INDEX_START -->
- [irish-trad-session_2026-09-15_11-30-05.pdf](irish-trad-session_2026-09-15_11-30-05.pdf)
- [irish-trad-session_2026-09-13_20-48-41.pdf](irish-trad-session_2026-09-13_20-48-41.pdf)
<!-- ARCHIVE_INDEX_END -->

<link rel="shortcut icon" type="image/png" href="{{ '/favicon.png?' | relative_url }}">
